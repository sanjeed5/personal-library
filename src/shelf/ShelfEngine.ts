...
export class ShelfEngine {
  ...
  private updateBrowseMotion(delta: number) {
    ...
    switch (phase) {
      ...
      case "settle-next":
        ...
        this.ensureCustomCover(motionIndex);
  ...
  focusBook(index = this.activeIndex) {
    ...
    // Search and catalog jumps should feel immediate even across 200 volumes.
    // Settle any in-flight browse pose, present the target, then focus it.
    if (this.presentedIndex !== null && this.presentedIndex !== next) {
      this.commitBookPose(
    ...
    if (this.motionBookIndex !== null && this.motionBookIndex !== next) {
      this.commitBookPose(
        this.runtimeBooks[this.motionBookIndex],
        shelvedBookPose(this.motionLayout),
        false,
      );
    }
